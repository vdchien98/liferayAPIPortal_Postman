...
import com.liferay.treinamento.contacto.model.ContactoTable;
...
import com.liferay.treinamento.contacto.service.persistence.ContactoUtil;
 ...
 * @generated
 */
@Component(service = {ContactoPersistence.class, BasePersistence.class})
public class ContactoPersistenceImpl
	...
	public ContactoPersistenceImpl() {
		...
		setTable(ContactoTable.INSTANCE);
	}
	...
	public void cacheResult(List<Contacto> contactos) {
		if ((_valueObjectFinderCacheListThreshold == 0) ||
			((_valueObjectFinderCacheListThreshold > 0) &&
			 (contactos.size() > _valueObjectFinderCacheListThreshold))) {
	...
	public void clearCache() {
		...
		finderCache.clearCache(ContactoImpl.class);
	}
	...
	public void clearCache(Contacto contacto) {
		entityCache.removeResult(ContactoImpl.class, contacto);
	}
	...
	protected void cacheUniqueFindersCache(
		...
		finderCache.putResult(
			_finderPathFetchByUUID_G, args, contactoModelImpl);
	}
	...
	public Contacto updateImpl(Contacto contacto) {
		...
		if (isNew && (contacto.getCreateDate() == null)) {
			if (serviceContext == null) {
				contacto.setCreateDate(date);
			}
			else {
				contacto.setCreateDate(serviceContext.getCreateDate(date));
			}
		}

		if (!contactoModelImpl.hasSetModifiedDate()) {
			if (serviceContext == null) {
				contacto.setModifiedDate(date);
			}
			else {
				contacto.setModifiedDate(serviceContext.getModifiedDate(date));
			}
		}
		...
		entityCache.putResult(
			ContactoImpl.class, contactoModelImpl, false, true);
	...
	public void activate() {
		...
		_setContactoUtilPersistence(this);
	}

	@Deactivate
	public void deactivate() {
		_setContactoUtilPersistence(null);

		entityCache.removeCache(ContactoImpl.class.getName());
	}

	private void _setContactoUtilPersistence(
		ContactoPersistence contactoPersistence) {

		try {
			Field field = ContactoUtil.class.getDeclaredField("_persistence");

			field.setAccessible(true);

			field.set(null, contactoPersistence);
		}
	...
	@Reference
	private ContactoModelArgumentsResolver _contactoModelArgumentsResolver;
